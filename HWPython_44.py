# Задача 44:
# В ячейке ниже представлен код генерирующий DataFrame, 
# которая состоит всего из 1 столбца. Ваша задача перевести его 
# в one hot вид. Сможете ли вы это сделать без get_dummies?

# import random
# lst = ['robot'] * 10
# lst += ['human'] * 10
# random.shuffle(lst)
# data = pd.DataFrame({'whoAmI':lst})
# data.head()
 


import pandas as pd
import random
lst = ['robot'] * 10
lst += ['human'] * 10
random.shuffle(lst)
data = pd.DataFrame({'whoAmI': lst})
categories = data['whoAmI'].unique()
category_dict = {category: [1 if cat == category else 0 for cat in categories] for category in categories}
one_hot_data = pd.DataFrame(data['whoAmI'].apply(lambda x: category_dict[x]).tolist(), columns=categories)
data = pd.concat([data, one_hot_data], axis=1)
data.head()